...
public class BridgeUtil {
	...
	public BridgeUtil(BridgeWebView webView){
		this.webView = webView;

		JS_HANDLE_MESSAGE_FROM_JAVA = "javascript:" + webView.getBridgeName() + "._handleMessageFromNative('%s');";
		JS_FETCH_QUEUE_FROM_JAVA = "javascript:" + webView.getBridgeName() + "._fetchQueue();";
	}

	public static BridgeUtil getInstance(BridgeWebView webView){
		if(one != null && one.webView == webView){
			return one;
		}
		one = new BridgeUtil(webView);
		return one;
	}

	public String parseFunctionName(String jsUrl){
		return jsUrl.replace("javascript:" + webView.getBridgeName() + ".", "").replaceAll("\\(.*\\);", "");
	}
	...
	public String assetFile2Str(Context c, String urlStr){
		...
		try{
            ...
            String line = null;
            StringBuilder sb = new StringBuilder();
			sb.append("var bridgeName = \"").append(webView.getBridgeName()).append("\";");
            do {
